...
export const PlatformButton = ({ platform, url, label, className = "" }: PlatformButtonProps) => {
    ...
    return (
        <a
            ...
            className={`group inline-flex flex-col items-center justify-center gap-2 px-4 py-3 w-[150px] h-[90px] ${config.bg} text-white rounded-xl ${config.hover} transition-all duration-300 shadow-lg ${config.shadow} hover:scale-105 hover:shadow-2xl flex-shrink-0 ${className}`}
        >
            <div className="bg-white/95 rounded-lg p-1.5 flex items-center justify-center shadow-md backdrop-blur-sm">
                <img
                    src={imageSrc}
                    alt={`${label} logo`}
            ...
            <span className="font-bold text-xs text-center whitespace-nowrap tracking-wide">{label}</span>
        </a>
    );
};
