...
export const PlatformButton = ({ platform, url, label, className = "" }: PlatformButtonProps) => {
    ...
    return (
        <a
            ...
            className={`group inline-flex items-center justify-center gap-2 px-3 py-2 w-[150px] ${config.bg} text-white rounded-lg ${config.hover} transition-all duration-300 shadow-lg ${config.shadow} hover:scale-105 hover:shadow-xl flex-shrink-0 ${className}`}
        >
            <div className="bg-white/95 rounded p-1 flex items-center justify-center shadow-sm backdrop-blur-sm">
                <img
                    src={imageSrc}
                    alt={`${label} logo`}
